...
class InterbotixRobot(object):
    ...
    def __init__(self, robot_name, mrd=None, robot_model=None, moving_time=2.0, accel_time=0.3, gripper_pressure=0.5, use_time=True):
        ...
        srv_robot_info = rospy.ServiceProxy("get_robot_info", RobotInfo)                                              # Create Service Proxies
        self.srv_set_op = rospy.ServiceProxy("set_operating_modes", OperatingModes)
        self.srv_set_register = rospy.ServiceProxy("set_motor_register_values", RegisterValues)
        ...
        self.pub_joint_commands = rospy.Publisher("joint/commands", JointCommands, queue_size=100)                    # ROS Publisher to command the arm
        self.pub_single_command = rospy.Publisher("single_joint/command", SingleCommand, queue_size=100)              # ROS Publisher to command a specified joint
        ...
        if (mrd is not None):
            ...
            self.pub_gripper_command = rospy.Publisher("/gripper/command", Float64, queue_size=100)                    # ROS Publisher to command the gripper
            self.pub_joint_traj = rospy.Publisher("/arm_controller/joint_trajectory", JointTrajectory, queue_size=100) # ROS Pubilsher to command Cartesian trajectories
